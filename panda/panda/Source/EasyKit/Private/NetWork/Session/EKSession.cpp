...
AEKSession::AEKSession(const FObjectInitializer& ObjectInitializer)
: Super(ObjectInitializer)
{
	if (!HasAnyFlags(RF_ClassDefaultObject))
	{
		OnCreateSessionCompleteDelegate = FOnCreateSessionCompleteDelegate::CreateUObject(this, &AEKSession::OnCreateSessionComplete);
		OnDestroySessionCompleteDelegate = FOnDestroySessionCompleteDelegate::CreateUObject(this, &AEKSession::OnDestroySessionComplete);

		OnFindSessionsCompleteDelegate = FOnFindSessionsCompleteDelegate::CreateUObject(this, &AEKSession::OnFindSessionsComplete);
		OnJoinSessionCompleteDelegate = FOnJoinSessionCompleteDelegate::CreateUObject(this, &AEKSession::OnJoinSessionComplete);

		OnStartSessionCompleteDelegate = FOnStartSessionCompleteDelegate::CreateUObject(this, &AEKSession::OnStartOnlineGameComplete);
	}
}
 ...
 */
void AEKSession::OnStartOnlineGameComplete(FName SessionName, bool bWasSuccessful)
{
...
void AEKSession::HandleMatchHasStarted()
{
 ...
 *
 */
void AEKSession::HandleMatchHasEnded()
{
	// start online game locally and wait for completion
	IOnlineSubsystem* OnlineSub = IOnlineSubsystem::Get();
	if (OnlineSub)
...
EOnlineAsyncTaskState::Type AEKSession::GetSearchResultStatus(int32& SearchResultIdx, int32& NumSearchResults)
{
	SearchResultIdx = 0;
	NumSearchResults = 0;
 ...
 */
const TArray<FOnlineSessionSearchResult> & AEKSession::GetSearchResults() const
{ 
 ...
 * @param bWasSuccessful true if the async action completed without error, false if there was an error
 */
void AEKSession::OnCreateSessionComplete(FName SessionName, bool bWasSuccessful)
{
 ...
 * @param bWasSuccessful true if the async action completed without error, false if there was an error
 */
void AEKSession::OnDestroySessionComplete(FName SessionName, bool bWasSuccessful)
{
...
void AEKSession::DelayedSessionDelete()
{
	IOnlineSubsystem* OnlineSub = IOnlineSubsystem::Get();
	if (OnlineSub)
	{
		...
		else
		{
			// Retry shortly
			FTimerHandle Retry;
			//GetWorldTimerManager().SetTimer(Retry, this, &AEKSession::DelayedSessionDelete, 1.f);
		}
	}
}

bool AEKSession::HostSession(TSharedPtr<FUniqueNetId> UserId, FName SessionName, const FString & GameType, const FString & MapName, bool bIsLAN, bool bIsPresence, int32 MaxNumPlayers)
{
	...
	if (OnlineSub)
	{
		...
		{
			HostSettings = MakeShareable(new FEKOnlineSessionSettings(bIsLAN, bIsPresence, MaxPlayers));
...
void AEKSession::OnFindSessionsComplete(bool bWasSuccessful)
{
...
void AEKSession::ResetBestSessionVars()
{
	CurrentSessionParams.BestSessionIdx = -1;
}

void AEKSession::ChooseBestSession()
{
...
void AEKSession::StartMatchmaking()
{
	ResetBestSessionVars();
	ContinueMatchmaking();
}

void AEKSession::ContinueMatchmaking()
{	
	ChooseBestSession();
...
void AEKSession::OnNoMatchesAvailable()
{
	UE_LOG(LogOnlineGame, Verbose, TEXT("Matchmaking complete, no sessions available."));
	SearchSettings = NULL;
}

void AEKSession::FindSessions(TSharedPtr<FUniqueNetId> UserId, FName SessionName, bool bIsLAN, bool bIsPresence)
{
	IOnlineSubsystem* OnlineSub = IOnlineSubsystem::Get();
	if (OnlineSub)
	{
		...
		if (Sessions.IsValid() && CurrentSessionParams.UserId.IsValid())
		{
			SearchSettings = MakeShareable(new FEKOnlineSearchSettings(bIsLAN, bIsPresence));
...
bool AEKSession::JoinSession(TSharedPtr<FUniqueNetId> UserId, FName SessionName, int32 SessionIndexInSearchResults)
{
	bool bResult = false;
...
bool AEKSession::JoinSession(TSharedPtr<FUniqueNetId> UserId, FName SessionName, const FOnlineSessionSearchResult & SearchResult)
{
	bool bResult = false;
 ...
 * @param bWasSuccessful true if the async action completed without error, false if there was an error
 */
void AEKSession::OnJoinSessionComplete(FName SessionName, EOnJoinSessionCompleteResult::Type Result)
{
...
bool AEKSession::TravelToSession(int32 ControllerId, FName SessionName)
{
	IOnlineSubsystem* OnlineSub = IOnlineSubsystem::Get();
	if (OnlineSub)
	{
