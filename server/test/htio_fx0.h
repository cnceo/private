...
class ServiceBase{
public:
			ServiceBase(work_handler& w,myalloc& a,size_t ios=1,size_t works=1,size_t rb_size=1460)
...
public:
	...
	virtual void	send(const packet_t& p){
		if(_sh)_sh->send((void*)&p,get_packet_length(p));
	}
protected:
...
public:
					SessionHandlerBase(_Ax& ax):_ax(ax),_s(nullptr){}
