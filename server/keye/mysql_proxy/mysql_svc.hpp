...
namespace keye{
class mysql_svc;
class mysql_alloc:public htio_alloc{
public:
	virtual void*	allocate(size_t _Count){return _a.allocate(_Count);}
	virtual void	deallocate(void* _Ptr, size_t){_a.deallocate((char*)_Ptr,0);}
private:
	std::allocator<char> _a;
};

class mysql_svc_handler:public work_handler{
public:
...
public:
				mysql_svc(mysql_proxy_impl* proxy,unsigned char threads=1)
					:_w(*this),_s(_w,_a,1,threads),_proxy(proxy){}
	void		start(){
		_s.run();
	}
